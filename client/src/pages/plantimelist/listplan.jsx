...
const ListPlan = () => {
  ...
  const getColorCode = (productName, colorName) => {
    ...
    const directMatch = Object.keys(colorMap).find(
      (code) =>
        colorName && colorName.toLowerCase().includes(code.toLowerCase())
    );
  ...
  const handleShowPlanTimeDetail = async (plantimeId) => {
    try {
      const response = await axios.get(`/api/get/plantime/${plantimeId}`);

      if (
        !response.data ||
        !response.data.planTimes ||
        response.data.planTimes.length === 0
      ) {
        toast.info("ℹ️ ไม่มีข้อมูล Plan Time สำหรับ Plantime นี้");
        return;
      }
      ...
      localStorage.setItem(
        ...
        JSON.stringify({
          ...
          productName: response.data.productName, // เพิ่มบรรทัดนี้
          colorName: response.data.colorName, // เพิ่มบรรทัดนี้
        })
      );
      ...
      toast.error(
        error.response?.data?.message || "❌ ไม่สามารถดึงข้อมูลแผนเวลาได้"
      );
    }
  };
  ...
  return (
    <div className="listplan-container">
      <div className="listplan-header-section">
        ...
        <div className="listplan-status-summary">
          <div className="listplan-status-card">
            <span className="listplan-status-count">
                ...
                return (
                  ...
                  >
                    ...
                    <td className="listplan-time-cell">
                      {plan.startTime ? (
                        <span className="listplan-time-value">
                          {plan.startTime}
                        </span>
                      ) : (
                    ...
                    <td className="listplan-status-cell">
                      <span
                        className={`listplan-status-badge listplan-status-${
                          plan.status || "pending"
                        }`}
                      >
                    ...
                    <td className="listplan-action-cell">
                      <button
                        ...
                        onClick={() =>
                          handleShowPlanTimeDetail(plan.plantime_id)
                        } // ส่ง plantime_id
      ...
      <div className="listplan-results-info">
        ...
        {(searchTerm || statusFilter !== "all") && (
          <span className="listplan-filter-indicator">(มีการกรองข้อมูล)</span>
        )}
      </div>
    </div>
  );
