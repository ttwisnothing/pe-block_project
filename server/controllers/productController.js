...
export const addProduct = async (req, res) => {
    const { product_name, status, resin, foaming, color, bRound, bUse, chemicals } = req.body;
    ...
    const query = `
        INSERT INTO product_master (
            product_name, status, resin, foaming, color, bRound, bUse, ${chemicalColumns.join(", ")}
        ) VALUES (?, ?, ?, ?, ?, ${chemicalColumns.map(() => "?").join(", ")})
    `;

    try {
        // บันทึกข้อมูลลงฐานข้อมูล
        await db.query(query, [product_name, status, resin, foaming, color, bRound, bUse, ...chemicalValues]);
